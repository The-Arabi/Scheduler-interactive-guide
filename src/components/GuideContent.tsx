...
import { CheckCircle, ChevronRight, ChevronLeft, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 border-r border-slate-200">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        {/* Progress Bar */}
        <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden flex select-none">
          {chapters.map((ch, idx) => {
        ...
        <h2 className="text-lg font-bold text-slate-900 mt-3 flex items-center gap-2 font-serif text-left">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 text-left">
        {/* Objective Toast */}
        <div className={`p-4 rounded border transition-all duration-350 ${
          isCompleted 
          ...
          <div className="flex items-start gap-3 text-left">
            ...
            <div className="flex-1 text-left">
              ...
              <p className="text-sm mt-0.5 font-medium leading-relaxed font-sans text-left">
                {currentChapter.objectiveText}
              </p>
              {isCompleted && (
                <p className="text-xs mt-1.5 text-emerald-700 font-bold">
              ...
              {!isCompleted && onToggleComplete && (
                <button
                  ...
                  className="mt-3 px-3 py-1.5 text-[11px] font-semibold text-[#0a304e] hover:bg-[#0a304e]/10 rounded border border-[#0a304e]/20 transition-colors cursor-pointer"
                >
                  Mark done manually
                </button>
              )}
      ...
      <div className="bg-white border-t border-gray-200 p-4 shrink-0 flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 rounded text-xs font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-transparent cursor-pointer"
        >
        ...
        {isCompleted && activeChapterIdx < chapters.length - 1 ? (
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 border border-slate-200 text-slate-600 rounded text-xs font-medium hover:bg-slate-200 disabled:opacity-40 disabled:hover:bg-slate-100 cursor-pointer"
          >
            <span>Next Step</span>
