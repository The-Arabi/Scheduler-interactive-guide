...
import { BookOpen, Search, Table, HelpCircle, CornerDownRight, Tag } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-4 gap-4">
        ...
        <div className="relative max-w-xs">
          ...
          <input
            ...
            className="w-full text-xs pl-9 pr-3 py-1.5 border border-slate-200 rounded-lg placeholder:text-slate-400 focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="space-y-3.5">
        {filteredFields.length === 0 ? (
          ...
          filteredFields.map((field, i) => (
            <div key={i} className="flex gap-2 text-xs border-b border-slate-100 pb-3 last:border-b-0 last:pb-0 font-medium">
              <div className="w-1/4 select-none flex items-center gap-2 font-mono font-bold text-slate-800 shrink-0">
                <CornerDownRight className="w-4 h-4 text-slate-300" />
                <span>{field.id}</span>
              </div>
              <div className="flex-1 text-slate-600 leading-relaxed pt-0.5">
                {field.def}
              </div>
            </div>
          ))
