...
if (fs.existsSync(lowercaseSrc)) {
  ...
  // Scan for any case-mismatched variants of index.css inside lowercaseSrc
  try {
    const files = fs.readdirSync(lowercaseSrc);
    for (const file of files) {
      if (file.toLowerCase() === 'index.css' && file !== 'index.css') {
        const badPath = path.join(lowercaseSrc, file);
        const goodPath = path.join(lowercaseSrc, 'index.css');
        console.log(`[Pre-build] Renaming case-mismatched CSS file "${file}" to "index.css".`);
        fs.renameSync(badPath, goodPath);
      }
    }
  } catch (e) {
    console.error('[Pre-build] Failed scanning/renaming index.css variant:', e.message);
  }
}
